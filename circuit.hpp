...
public:
	...
	TxCircuit(
		...
		tx_amount(pb, 16, FMT(annotation_prefix, ".amount")),
		...
		sig_m(flatten({tx_from_idx, tx_to_idx, tx_amount.bits, sig_nonce.bits, tx_amount.bits})),
		//
		// Calculate hash used for signature
		// 		M = (from_idx, to_idx, sig_nonce, tx_amount)
		//		A = (from.x, from.y)
		...
		m_leaf_after_from(pb, libsnark::ONE, {from_pubkey.x, from_pubkey.y, m_balance.A, next_nonce}, FMT(annotation_prefix, ".leaf_after_from")),
		...
		m_leaf_after_to(pb, libsnark::ONE, {to_pubkey.x, to_pubkey.y, m_balance.B, to_nonce}, FMT(annotation_prefix, ".leaf_after_to")),
	...
	void generate_r1cs_witness( const snasma::TxProof& proof )
	{
		...
		tx_amount.bits.fill_with_bits_of_ulong(this->pb, (unsigned long)proof.stx.tx.amount);
